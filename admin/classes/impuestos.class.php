...
class Impuestos extends Main
{
	...
	public function Enumerate()
	{
		...
		{
			$this->Util()->DB()->setQuery('SELECT COUNT(*) FROM impuestos WHERE 1 '.$filtro);
		}
		else
		{
			$this->Util()->DB()->setQuery('SELECT COUNT(*) FROM impuestos WHERE 1 AND mostrar = "Si" '.$filtro);
		}
		...
		if($_SESSION["loginKey"] == 1)
		{
			...
			LEFT JOIN socio ON socio.socioId = impuestos.idSocio WHERE 1 '.$filtro.'														
																	ORDER BY idVenta DESC '.$sql_add);
		}
		else
		{
			$this->Util()->DB()->setQuery('SELECT * FROM impuestos 
			LEFT JOIN socio ON socio.socioId = impuestos.idSocio WHERE 1 AND mostrar = "Si"  '.$filtro.'														
																	ORDER BY idVenta DESC '.$sql_add);
		}
	...
	{
		$this->Util()->DB()->setQuery("SELECT * FROM impuestos WHERE idVenta = '".$this->idVenta."'");
	...
	public function Edit()
	{
		...
		$this->Util()->DB()->setQuery("
			SELECT idEmpresa FROM impuestos WHERE idVenta = '".$this->idVenta."'");
...
//		$cantidad = $this->Util()->DB()->GetSingle();
		...
		if($this->status == "pagado")
		{
				...
				UPDATE
					empresa
				SET moduloImpuestos = 'Si', venImpuestos = '".$datePost."' WHERE empresaId = ".$idEmpresa);
	...
	public function Save()
	{
		...
		$subject = 'Se activo un modulo de nomina ';
		
		$this->Util()->DBSelect($this->idEmpresa)->setQuery("SELECT * FROM ".DB_PREFIX.$this->idEmpresa.".rfc LIMIT 1");
		$rfc = $this->Util()->DBSelect($this->idEmpresa)->GetRow();
		$razonSocial = urldecode($rfc["razonSocial"]);
		
		$body = "Para la empresa ".$razonSocial.".\r\n";
