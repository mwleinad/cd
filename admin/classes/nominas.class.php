...
class Nominas extends Main
{
	...
	public function Enumerate()
	{
		...
		{
			$this->Util()->DB()->setQuery('SELECT COUNT(*) FROM nominas WHERE 1 '.$filtro);
		}
		else
		{
			$this->Util()->DB()->setQuery('SELECT COUNT(*) FROM nominas WHERE 1 AND mostrar = "Si" '.$filtro);
		}
		...
		if($_SESSION["loginKey"] == 1)
		{
			...
			LEFT JOIN socio ON socio.socioId = nominas.idSocio WHERE 1 '.$filtro.'														
																	ORDER BY idVenta DESC '.$sql_add);
		}
		else
		{
			$this->Util()->DB()->setQuery('SELECT * FROM nominas 
			LEFT JOIN socio ON socio.socioId = nominas.idSocio WHERE 1 AND mostrar = "Si"  '.$filtro.'														
																	ORDER BY idVenta DESC '.$sql_add);
		}
	...
	{
		$this->Util()->DB()->setQuery("SELECT * FROM nominas WHERE idVenta = '".$this->idVenta."'");
	...
	public function Edit()
	{
		...
		$this->Util()->DB()->setQuery("
			SELECT idEmpresa FROM nominas WHERE idVenta = '".$this->idVenta."'");
...
//		$cantidad = $this->Util()->DB()->GetSingle();
		...
		if($this->status == "pagado")
		{
				...
				UPDATE
					empresa
				SET moduloNomina = 'Si', venNomina = '".$datePost."' WHERE empresaId = ".$idEmpresa);
