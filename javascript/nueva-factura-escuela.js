...
function retornarAVentas(){
	location.href="facturacion/reporte-ventas";
	}
...
function VistaPreviaComprobante()
{
	...
	if(!confirm(message))
  {
	...
	$('totalesDesglosadosDiv').innerHTML = '<div align="center"><img src="'+WEB_ROOT+'/images/load.gif" /><br>Generando Vista Previa, este proceso puede tardar unos segundos</div>';
	...
	//adddenda
	if($('idPedido')) var idPedido = $('idPedido').value;
	else var idPedido = 0;

	if($('idSolicitudDePago')) var idSolicitudDePago = $('idSolicitudDePago').value;
	else var idSolicitudDePago = 0;

	if($('referencia')) var referencia = $('referencia').value;
	else var referencia = 0;

	if($('idProveedor')) var idProveedor = $('idProveedor').value;
	else var idProveedor = 0;

	if($('finalidad')) var finalidad = $('finalidad').value;
	else var finalidad = 0;

	if($('uppDestino')) var uppDestino = $('uppDestino').value;
	else var uppDestino = 0;

	new Ajax.Request(WEB_ROOT+'/ajax/sistema.php', 
	{
  	parameters: {nuevaFactura: nuevaFactura, observaciones: $('observaciones').value, type: "vistaPreviaComprobante", reviso: reviso, autorizo: autorizo, recibio: recibio, vobo: vobo, pago: pago, porcentajeISH: porcentajeISH, tiempoLimite:tiempoLimite, formatoNormal:formatoNormal, idPedido:idPedido, idSolicitudDePago:idSolicitudDePago, referencia:referencia, idProveedor:idProveedor, finalidad:finalidad, uppDestino:uppDestino, banco:banco, fechaDeposito:fechaDeposito},
		method:'post',
