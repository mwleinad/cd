...
class Folios extends Empresa{
	...
	public function setIdSucursal($value){
		$this->Util()->ValidateString($value, $max_chars=255, $minChars = 1, 'Sucursal');
...
*/		$this->email = $value;
	...
	function VerifyFolios($edit = false){
		...
		if(!$edit)
		{
			...
			if($result > 0)
			{
					$this->Util()->setError(20026, 'error', "No puedes agregar otra serie con el mismo Nombre.<br> Favor de editar el Folio Final de la serie original.");	
					$this->Util()->PrintErrors();
					return true;			
			}
		}
...
//		print_r($folios);
	...
	function EditFolios(){
		...
		if($this->VerifyFolios($edit = true)){
			return false;
		}
		...
		$this->Util()->DBSelect($_SESSION['empresaId'])->setQuery("
			UPDATE `serie` SET 
				...
				`noCertificado` = '".$this->getNoCertificado()."', 
				`email` = '".$this->getEmail()."',
				`sucursalAsignada` = '".$this->getSucursalId()."'
