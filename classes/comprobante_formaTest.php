...
class ComprobanteFormaTest extends Comprobante
{
	...
	function Generar($data, $notaCredito = false)
	{
		...
		if($data["fromAddenda"] == "Si")
		{
			if(!$data["idPedido"])
			{
				$vs->Util()->setError(10041, "error", "El campo Id Pedido es Requerido");
			}

			if(!$data["idProveedor"])
			{
				$vs->Util()->setError(10041, "error", "El campo Id Proveedor es Requerido");
			}

			if(!$data["idRecepcion"])
			{
				$vs->Util()->setError(10041, "error", "El campo Id Recepcion es Requerido");
			}
...
//			$timbradoFile = $root."/timbres/".$nufa.".xml";
			...
			if($data["fromAddenda"] == "Si")
			{
				$nufa = "SIGN_".$empresa["empresaId"]."_".$serie["serie"]."_".$data["folio"];
				$realSignedXml = $root.$nufa.".xml";
				include_once(DOC_ROOT."/classes/generate_xml_pepsico.php");
