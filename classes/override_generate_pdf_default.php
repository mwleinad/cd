...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(20,10,$data["comprobante"]["nombre"],0,0,'C');
		...
		$pdf->Image($rootQr.$nufa.'.png',176,8, 33, 33);		
		...
		$data["nodoEmisor"]["rfc"]["razonSocial"] = str_replace("&AMP;", "&", $data["nodoEmisor"]["rfc"]["razonSocial"]);
		$data["nodoEmisor"]["rfc"]["razonSocial"] = str_replace("&amp;", "&", $data["nodoEmisor"]["rfc"]["razonSocial"]);
		$data["nodoEmisor"]["rfc"]["razonSocial"] = str_replace("%MAS%", "+", $data["nodoEmisor"]["rfc"]["razonSocial"]);
...
//		$pdf->MultiCell(80,3,utf8_decode(urldecode($data["nodoReceptor"]["nombre"])),0);
		...
		$infoReceptor = str_replace("%MAS%", "+", $infoReceptor);
...
//		if($data["nodoEmisor"]["sucursal"]["sucursalActiva"] == 'no'){
			...
			$infoSucursal .= "\n".urldecode($data["nodoEmisor"]["sucursal"]["calle"]." ".$data["nodoEmisor"]["sucursal"]["noExt"]." ".$data["nodoEmisor"]["sucursal"]["noInt"]);
			...
			{
				$infoSucursal = urldecode($data["nodoEmisor"]["rfc"]["calle"]." ".$data["nodoEmisor"]["rfc"]["noExt"]." ".$data["nodoEmisor"]["rfc"]["noInt"]." ".$data["nodoEmisor"]["rfc"]["colonia"]);
				
				$infoSucursal .= "\n".utf8_decode(urldecode($data["nodoEmisor"]["rfc"]["municipio"]." ".$data["nodoEmisor"]["rfc"]["estado"]." ".$data["nodoEmisor"]["rfc"]["pais"]." CP: ".$data["nodoEmisor"]["rfc"]["cp"]));
...
/*		else
		...
		$count++;
			...
			{
				$concepto["descripcion"] .= " Cuenta predial:".$concepto["cuentaPredial"];
			}			
...
//		$pdf->CheckPageBreak(50);
		...
		{
			case "MXN": $moneda = "$ "; $tiposDeCambio = "Pesos"; $tiposDeCambioSufix = "M.N";break;
			case "USD": $moneda = "USD "; $tiposDeCambio = "Dolares"; $tiposDeCambioSufix = "";break;
			case "EUR": $moneda = "EUR "; $tiposDeCambio = "Euros"; $tiposDeCambioSufix = "";break;
			case "GTQ": $moneda = "GTQ "; $tiposDeCambio = "Quetzales"; $tiposDeCambioSufix = "";break;
		}
...
//echo $data["formaDePago"];
		...
		{
			$tasaIva = $totales["tasaIva"]."% IVA";
		}
		$pdf->MultiCell(20,3,$tasaIva,0,"C");
		$pdf->SetY($pdf->GetY()-3);
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
...
//print_r($totales);
		...
		if($totales["porcentajeISH"] != 0)
		{
			...
			$pdf->SetX($xstart);
			$pdf->SetTextColor(255, 255, 255);
			$pdf->MultiCell(20,3,"ISH",0,"C");
			$pdf->SetY($pdf->GetY()-3);
			$pdf->SetX($xstart+20);
			$pdf->SetTextColor(0,0,0);
			$pdf->MultiCell(31,3,$moneda.$this->Util()->CadenaOriginalPDFFormat($totales["ish"], true,false),0,"R",0);
		}
		...
		if($miEmpresa["donatarias"] == "Si" && $data["comprobante"]["tiposComprobanteId"] == TIPO_COMPROBANTE_DONATARIA)
		{
			$addDonataria = "\n".$miEmpresa["leyenda"]."\n # Autorizacion: ".$miEmpresa["noAutorizacion"]." Fecha de Autorizacion: ".$miEmpresa["fechaAutorizacion"];
		}
		
		$xstart = 2;
		$pdf->SetY($pdf->GetY()+20);
		$pdf->SetX($xstart);
		$pdf->MultiCell(207,3,utf8_decode("ESTE DOCUMENTO ES UNA REPRESENTACION IMPRESA DE UN CFDI ".$addDonataria),0,"C");
