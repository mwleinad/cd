...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(5,5,"6",1,0,'C');
		...
		$pdf->SetY(38);
		$pdf->SetX(6);
		$pdf->MultiCell(70,8,"Vendido A",0);
 		$pdf->SetTextColor(0,0,0);
...
//		if($data["nodoEmisor"]["sucursal"]["sucursalActiva"] == 'no'){
		...
		else
		{
			...
			$nb = $pdf->WordWrap($infoSucursal, 100);
				$pdf->Row(
					array( utf8_decode(urldecode("# Registro: ".$data["noRegistro"]."\n".$infoSucursal."\nForma de Pago:".$data["formaDePago"]."\n".$add))
					), 3
				);
		...
		$pdf->Cell(20,8,"Tipo",0,0,"C");
		$pdf->Cell(20,8,"Peso",0,0,"C");
		$pdf->Cell(73,8,"Descripcion",0,0,"C");
		...
		$pdf->SetWidths(array(5,15, 15, 20, 20, 20, 66, 22, 22));
		$pdf->SetAligns(array('L','L', 'L', 'L', 'L', 'L', 'L', 'R', 'R'));
		$pdf->SetFont('courier','',7);
		$xstart = 15;
		$count = 1;
	    ...
	    $pdf->Row(
				array(
					"   -",
					$this->Util()->CadenaOriginalVariableFormat($concepto["peso"],false,false),
					...
					$this->Util()->CadenaOriginalVariableFormat($concepto["tipoGanado"],false,false), 
					$this->Util()->CadenaOriginalVariableFormat($concepto["cantidad"],true,false), 
...
//echo $data["formaDePago"];
		...
		$pdf->SetX(25);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(120,3,$this->Util()->DecodeVal("Metodo de Pago: ".$data["metodoDePago"]),0);
...
//print_r($totales);
		...
		if($totales["porcentajeISH"] != 0)
		{
			...
			$pdf->SetX($xstart+20);
			$pdf->SetTextColor(0,0,0);
			$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($totales["ish"], true,false),0,"R",0);
		}
  	...
  	$pdf->Cell(5,5,"1",1,0,'C');
		...
		$pdf->MultiCell(60,3,utf8_decode("# Guia de Traslado:".urldecode($data["noGuiaTraslado"])),0,"L",0);

		$pdf->SetY($y);
		$pdf->SetX(100);
		$pdf->MultiCell(60,3,utf8_decode(urldecode("Finalidad del Ganado: ".$data["finalidad"])),0,"L", 0);
		...
		$y = $pdf->GetY();
		$pdf->SetY($y);
		$pdf->MultiCell(90,3,utf8_decode(urldecode("UPP Origen: ".$data["uppOrigen"])),0,"L",0);
		
		$pdf->SetY($y);
		$pdf->SetX(100);
		
		$pdf->MultiCell(90,3,utf8_decode(urldecode("UPP Destino: ".$data["uppDestino"])),0,"L",0);
		...
		$y = $pdf->GetY();
		$pdf->SetY($y);
		$pdf->MultiCell(90,3,utf8_decode(urldecode("Patente del Vendedor: ".$data["patenteVendedor"])),0,"L",0);
...
//		$pdf->MultiCell(207,3,"Cadena Original: \n".$cadena["cadenaOriginal"],0,"L",1);
  	...
  	$pdf->Cell(5,5,"2",1,0,'C');
		...
		$pdf->MultiCell(207,3,"____________________________________________________",0,"C",1);
		$pdf->MultiCell(207,3,"Firma del Vendedor",0,"C",1);
