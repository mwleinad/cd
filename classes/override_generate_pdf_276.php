...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(20,10,$data["comprobante"]["nombre"],0,0,'C');
		...
		if($_SESSION["empresaId"] == 357  || $_SESSION["empresaId"] == 356)
		{
			//$fecha = "2014-11-30 23:29:52";
		}
...
//		$pdf->CheckPageBreak(50);
		...
		if($_SESSION["impuestos"])
		{
			...
			*/
			...
			{
				if($impuesto["impuesto"] == "ISN")
				{
					$isn = $impuesto["importe"] * count($_SESSION["conceptos"]);
				}
...
//echo $data["formaDePago"];
		...
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($totales["subtotal"] + $isn, true,false),0,"R",0);

		$xstart = 155;
		$pdf->SetX($xstart);
		$pdf->SetTextColor(255, 255, 255);
		$pdf->MultiCell(20,3,"ISN",0,"C",0);
		$pdf->SetY($pdf->GetY()-3);
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($isn, true,false),0,"R",0);
