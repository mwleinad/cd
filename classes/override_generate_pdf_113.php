...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(20,10,$data["comprobante"]["nombre"],0,0,'C');
		...
		$pdf->SetX($xstart);
		$pdf->MultiCell(150,5,utf8_decode("GRUPO GARCIA Y GARCIA, S.A. DE C.V."),0, "C");


		$pdf->SetFont('verdana','',7);
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(150,3,utf8_decode("AVENIDA MISIONEROS No. 4 COL. SAN PEDRO MIRADOR, TUXTLA GUTIERREZ, CHIAPAS"),0, "C");

		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(150,8,"RFC: GGG-841207-3k1",0, "C");
