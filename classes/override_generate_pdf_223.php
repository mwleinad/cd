...
/*			$setY = $pdf->GetY();
			...
			{
				$totalImpuesto += $impuesto["importe"];
/*				$nb = $pdf->WordWrap($impuesto["impuesto"], 80);
			...
			$totalImpuesto = $totalImpuesto * count($_SESSION["conceptos"]);
		//check page break
//		$pdf->CheckPageBreak(50);*/
		}
...
//echo $data["formaDePago"];
		...
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($totales["subtotal"] + $totalImpuesto, true,false),0,"R",0);
		...
		$pdf->SetX($xstart);
		$pdf->SetTextColor(255, 255, 255);
		$pdf->MultiCell(20,3,"2% Nomina",0,"C");
		$pdf->SetY($pdf->GetY()-3);
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($totalImpuesto, true,false),0,"R",0);
