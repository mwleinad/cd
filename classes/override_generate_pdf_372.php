...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(20,10,$data["comprobante"]["nombre"],0,0,'C');
		...
		$pdf->SetX($xstart);
		$pdf->MultiCell(150,5,utf8_decode("TEIDY CHIU LARA"),0, "C");


		$pdf->SetFont('verdana','',7);
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(150,3,utf8_decode("AV ROSARIO L CASTELLANOS M 11 INT 3, EL PICHICHI, PIJIJIAPAN, 30540, CHIAPAS, MEXICO"),0, "C");

		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(150,8,"RFC: CILT711117CS8",0, "C");
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($x);
		$pdf->MultiCell(54,3,utf8_decode("00001000000302777122"),1, "C", 1);
...
//		$pdf->MultiCell(80,3,utf8_decode(urldecode($data["nodoReceptor"]["nombre"])),0);
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($x);
		$pdf->MultiCell(129,3,utf8_decode("AVENIDA MISIONEROS No. 4 COL SAN PEDRO MIRADOR\nTUXTLA GUTIERREZ, CHIAPAS\n".$data["pdfAutomatico"]),1, "C", 1);
		...
		$pdf->SetFont('courier','',7);
			...
			if($existImpuesto > 0)
			{
				...
				foreach($_SESSION["impuestos"] as $impuesto)
				{
					...
					if($impuesto["tipo"] == "deduccion")
					{
						...
						if(strpos($impuesto["impuesto"], "16%") !== 0)
						{
							$impuesto["impuesto"] = $impuesto["tasa"]."% ".$impuesto["impuesto"];
						}
			...
			if($existImpuesto > 0)
			{
				...
				foreach($_SESSION["impuestos"] as $impuesto)
				{
					if($impuesto["tipo"] == "retencion")
					{
						$suma += round($impuesto["importe"],2) * count($_SESSION["conceptos"]);
						if($impuesto["tasaIva"] > 0)
						{
							if(strpos($impuesto["impuesto"], "16%"))
							{
