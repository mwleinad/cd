...
class Override extends Comprobante
{
	function GeneratePDF($data, $serie, $totales, $nodoEmisor, $nodoReceptor, $nodosConceptos,$empresa, $cancelado = 0, $vistaPrevia = 0)
	{
  	...
  	$pdf->Cell(20,10,$data["comprobante"]["nombre"],0,0,'C');
		...
		{
			//$pdf->Image($rootQr.$nufa,$xstart+160,$setY+15, 27.5, 27.5,'PNG'); 		
			$pdf->Image($rootQr.$nufa,180,8,28,28,'PNG'); 		
		}
		else if(!file_exists($rootQr.$nufa))
		{
...
//		$pdf->Image($rootQr.$nufa.'.png',180,8, 28, 28);		
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(70,3,"No. Aprob. SICOFI",0);

		$xstart = 160;
		$pdf->SetY(10);
		$pdf->SetX($xstart);
		$pdf->SetTextColor(200, 0, 0);
		$pdf->MultiCell(18,3,$data["serie"]["serie"],0,0,'R');

		$pdf->SetTextColor(0, 0, 0);
		$xstart = 160;
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(18,3,$data["folio"],0,0,'R');

		$pdf->SetY($pdf->GetY());
		$pdf->SetX($xstart);
		$pdf->MultiCell(18,3,$data["serie"]["noAprobacion"],0,0,'R');
		...
		$infoReceptor .= "\n".urldecode($data["nodoReceptor"]["municipio"]." ".$data["nodoReceptor"]["estado"]." ".$data["nodoReceptor"]["pais"]." CP: ".str_pad($data["nodoReceptor"]["cp"],5, "0", STR_LEFT));
...
//$pdf->MultiCell(80,4,$infoReceptor,0);
		...
		//block sucursal
			...
			if(!$data["nodoEmisor"]["sucursal"]["municipio"])
			{
				$data["nodoEmisor"]["sucursal"]["municipio"] = $data["nodoEmisor"]["rfc"]["municipio"];
				$data["nodoEmisor"]["sucursal"]["estado"] = $data["nodoEmisor"]["rfc"]["estado"];
				$data["nodoEmisor"]["sucursal"]["pais"] = $data["nodoEmisor"]["rfc"]["pais"];
				$data["nodoEmisor"]["sucursal"]["cp"] = $data["nodoEmisor"]["rfc"]["cp"];
			}
			...
			if($data["sobreescribirFecha"] === true)
			{
				$fechaYHora = $data["fechaSobre"]." ".date("H:i:s");
			}
			else
			{
				$fechaYHora = date("d-m-Y H:i:s");
			}
				$infoSucursal .= "\nFecha y hora de expedicion: ".$fechaYHora;
			...
			$pdf->SetY(38);
			$pdf->SetX($xstart);
			$pdf->MultiCell(90,8,"Lugar de Expedicion",0);
	 		$pdf->SetTextColor(0,0,0);
...
//echo $data["formaDePago"];
		...
		if($totales["porcentajeISH"] != 0)
		{
			...
			$pdf->SetX($xstart);
			$pdf->SetTextColor(255, 255, 255);
			$pdf->MultiCell(20,3,$totales["porcentajeISH"]."% ISH",0,"C",1);
			$pdf->SetY($pdf->GetY()-3);
			$pdf->SetX($xstart+20);
			$pdf->SetTextColor(0,0,0);
			$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalVariableFormat($totales["ish"], true,false),0,"R",0);
		}
...
//print_r($data["serie"]);
		...
		$pdf->SetY($pdf->GetY()+5);
		$pdf->SetX($xstart);
		$pdf->MultiCell(207,3,utf8_decode("LA REPRODUCCION APOCRIFA DE ESTE COMPROBANTE CONSTITUYE UN DELITO EN LOS TERMINOS DE LAS DISPOSICIONES FISCALES. \nESTE COMPROBANTE TENDRA VIGENCIA DE DOS AÑOS CONTADOS A PARTIR DE LA FECHA DE APROBACION DE LA ASIGNACION DE FOLIOS LA CUAL ES: ".$data["serie"]["noCertificado"]." \nEFECTOS FISCALES AL PAGO."),0,"C");
