...
//		$pdf->CheckPageBreak(50);
		...
		if($_SESSION["impuestos"])
		{
			...
			{
				if($impuesto["impuesto"] != "MANO DE OBRA")
				{
...
//echo $data["formaDePago"];
		...
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($totales["subtotal"] + $suma, true,false),0,"R",0);
		...
		$pdf->SetX($xstart);
		$pdf->SetTextColor(255, 255, 255);
		$pdf->MultiCell(20,3,"Ret. 2% Mano de Obra",0,"C");
		$pdf->SetY($pdf->GetY()-3);
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($impuesto["importe"] * count($_SESSION["conceptos"]), true,false),0,"R",0);
				}
		}		
