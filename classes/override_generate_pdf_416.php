...
//		if($data["nodoEmisor"]["sucursal"]["sucursalActiva"] == 'no'){
		...
		$pdf->SetY($pdf->GetY()+2);
		$pdf->SetX($xstart);
		$pdf->Cell(20,8,"Numero.",0,0,"C");
		$pdf->Cell(20,8,"Embalaje",0,0,"C");
		$pdf->Cell(70,8,"Que el Remitente dice que contiene:",0,0,"C");
...
//		$pdf->CheckPageBreak(50);
		...
		if($_SESSION["impuestos"])
		{
			$setY = $pdf->GetY();
			$pdf->SetX($xstart);
			$pdf->Cell(40,8,"Otros Conceptos",0,0,"C");
			$pdf->Ln();
			$pdf->SetTextColor(0, 0, 0);
			...
			foreach($_SESSION["impuestos"] as $impuesto)
			{
				...
				//echo substr($impuesto["impuesto"], 0, 3);
				$descripcion = $impuesto["impuesto"];
				if(substr($impuesto["impuesto"], 0, 3) == "16%")
				{
					$descripcion = substr($impuesto["impuesto"], 3, strlen($impuesto["impuesto"]) - 3);
					$ivaImpuestos += $impuesto["importe"] * count($_SESSION["conceptos"]);
				}
				...
				if(substr($impuesto["impuesto"], 0, 3) != "16%")
				{
					$pdf->Row(
						array(
							...
							$this->Util()->CadenaOriginalVariableFormat("",false,false),
							$this->Util()->CadenaOriginalVariableFormat($descripcion,false,false), 
...
//echo $data["formaDePago"];
		...
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($totales["subtotal"] - $ivaImpuestos, true,false),0,"R",0);
		...
		$ivaTotal = $ivaImpuestos + $totales["iva"];
		...
		$pdf->SetX($xstart+20);
		$pdf->SetTextColor(0,0,0);
		$pdf->MultiCell(31,3,"$".$this->Util()->CadenaOriginalPDFFormat($ivaTotal, true,false),0,"R",0);
//print_r($totales);
		if($totales["retIva"] != 0)
		{
			$pdf->SetY($pdf->GetY());
