...
//		$pdf->MultiCell(80,3,utf8_decode(urldecode($data["nodoReceptor"]["nombre"])),0);
		...
		$pdf->SetY($pdf->GetY());
		$pdf->SetX($x);
		$pdf->MultiCell(129,3,utf8_decode("2a AVENIDA NORTE #755 BARRIO E. ZAPATA\nMOTOZINTLA, CHIAPAS\n".$data["fecha"]),1, "C", 1);
...
*/				$pdf->Ln();
				...
				foreach($_SESSION["impuestos"] as $impuesto)
				{
					if($impuesto["tipo"] == "impuesto")
					{
						...
						if($impuesto["importe"] * count($_SESSION["conceptos"]) == 0)
						{
							continue;
						}
			...
			if($existImpuesto > 0)
			{
				...
				foreach($_SESSION["impuestos"] as $impuesto)
				{
					if($impuesto["tipo"] == "deduccion")
					{
						...
						if($impuesto["importe"] * count($_SESSION["conceptos"]) == 0 && $impuesto["impuesto"] != "1% APORTACION AL MUNICIPIO PARA O.B.S")
						{
							continue;
						}
						...
						if($impuesto["impuesto"] != "1% APORTACION AL MUNICIPIO PARA O.B.S")
						{
							$pdf->Row(
								array(
									"",
