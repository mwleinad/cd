<?php
	...
	if($certNuevo == '')
		...
		$certNuevo = $util->GetNoCertificado($ruta_dir, $certNuevo);	
		$sqlQuery = 'UPDATE serie SET noCertificado = "'.$certNuevo.'" WHERE 1';
	...
	if($existe_certificado){	
		$expire = exec('openssl x509 -noout -in '.$ruta_dir.'/'.$nombreCertificado.'.cer.pem -dates');
		$exp = explode('=',$expire);
