<div id="divForm">
	<form id="editarUsuarioForm" name="editarUsuarioForm" method="post">
    <fieldset>
       	<div class="formLine">
			...
			<select name="tipoUsuario" id="tipoUsuario" class="largeInput wide2" onchange="CheckTipoUser()">
				<option value="">Seleccione</option>
   				{foreach from=$tipoUsuario item=tipo key=key}
					<option value="{$tipo}" {if $tipo == $post.type}selected{/if}>{$tipo|capitalize}</option>
  				{/foreach}
 			</select>
          	<div style="width:30%;float:left">Email:</div> 
            <input name="email" id="email" type="text" size="30" class="largeInput wide2" value="{$post.email}"/>          	
			...
			<select name="sucursalId" id="sucursalId" class="largeInput wide2">
				<option value="">Todas</option>
   				{foreach from=$sucursales item=sucursal key=key}
					<option value="{$sucursal.sucursalId}" {if $post.sucursalId == $sucursal.sucursalId}selected{/if}>{$sucursal.identificador|urldecode}</option>
  				{/foreach}
 			</select>
             <br />
             
             <div id="divPermisos" {if $post.type == "empleado"}style="display:none"{/if}>
             
             <div style="width:30%;float:left">Contrase&ntilde;a:</div> 
            <input name="password" id="password" type="text"  size="30" class="largeInput wide2" value="{$post.password}"/>
             ...
             <table width="100%">
                 ...
                 {foreach from=$modulos item=item key=key}
                    <tr>
                        <td><input type="checkbox" id="permisos[]" name="permisos[]" value="{$key}" {if isset($permisos)}{if in_array($key, $permisos)} checked="checked" {/if}{/if}>{$item}</td>
                        <td><input type="checkbox" value="view" name="row_{$key}[]" id="row_{$key}[]" {if isset($nuevosPermisos.$key)}{if in_array("view",$nuevosPermisos.$key)} checked="checked" {/if}{/if} /></td>
                        <td><input type="checkbox" value="create" name="row_{$key}[]" id="row_{$key}[]" {if isset($nuevosPermisos.$key)}{if in_array("create",$nuevosPermisos.$key)} checked="checked" {/if}{/if} /></td>
                        <td><input type="checkbox" value="edit" name="row_{$key}[]" id="row_{$key}[]" {if isset($nuevosPermisos.$key)}{if in_array("edit",$nuevosPermisos.$key)} checked="checked" {/if}{/if} /></td>
                        <td><input type="checkbox" value="delete" name="row_{$key}[]" id="row_{$key}[]" {if isset($nuevosPermisos.$key)}{if in_array("delete",$nuevosPermisos.$key)} checked="checked" {/if}{/if} /></td>
                    </tr>
                 {/foreach}
             </table>
            ...
            <div id="divEmpleado" {if $post.type != "empleado"}style="display:none"{/if}>
            ...
            <input name="clabe" id="clabe" type="text" size="30" class="largeInput wide2" value="{$post.clabe}"/>
          	...
          	<div style="width:30%;float:left">* Banco: (Favor de solicitar si necesita otro banco)</div>
            <select name="banco" id="banco"  class="largeInput wide2">
          	...
          	<div style="width:30%;float:left">Fecha Inicio Relación Laboral:
...
>
            ...
            <a class="button" id="editarUsuario" name="editarUsuario"><span>Actualizar</span></a>
        	<input type="hidden" name="type" value="saveUsuario" />
     		</div>
	</fieldset>
    <input type="hidden" id="usuarioId" name="usuarioId" value="{$post.usuarioId}" />
    <input type="hidden" name="type" value="saveEditUsuario" />
	</form>
</div>