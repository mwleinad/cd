...
*/		
		...
		$cents = ((string)$totales["total"] - floor((string)$totales["total"]))*100;
		$totales["total2"] = $totales["total"];
		$centavos = (string)$totales["total2"];
		$centavosLetra = $this->Util()->GetCents($centavos);
...
//print_r($totales);
		...
		$pdf->Output(DOC_ROOT.'/empresas/'.$_SESSION["empresaId"].'_pdf.pdf', "F");		
